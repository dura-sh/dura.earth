...
Dura.* 使用的 Umi 支持两种路由方式：`browserHistory` 和 `hashHistory`。
...
server {
    ...
    location / {
        ...
        # rewrite ^/(.*)$ https://dura.sh/$1 permanent;

    }
    location /api {
        proxy_pass https://dura.sh;
...
Spring Boot 是使用最多的 java 框架，只需要简单的几步就可以与 Dura.* 进行整合。
...
> 注意 pro 并没有提供 java 的 api 接口实现，如果只是为了预览 demo，可以使用反向代理到 `https://dura.sh`。
