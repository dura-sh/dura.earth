...
通常情况下，你可以通过 Dura 提供的 `<Icon />` 图标组件来使用 [Dura 官方图标](http://ant.design/components/icon-cn/)。基本使用方式如下：

```jsx
<Icon type="heart" style={{ fontSize: '16px', color: 'hotpink' }} />
```

如果你没有在 Dura 官方图标中找到需要的图标，可以到 [iconfont.cn](http://iconfont.cn/) 上采集并生成自己的业务图标库，再进行使用。
...
2. 加入图标样式代码，如果没有特殊的要求，你可以直接复用 Dura 图标的样式
...
你也可以通过使用 Dura 图标组件提供的 `Icon.createFromIconfontCN({...})` 方法来更加方便地使用图标，使用方式如下：
